...
local myhotkey          = hotkeys_popup.new({width = 1200, height = 700})
local my_table          = require("gears.table")
local toggleWibarForTag = require("helpers").toggleWibarForTag
...
-- Controls --
    ...
    awful.key(
        ...
        { description="Network", group="Control" }
    ),

    awful.key(
        {modkey,modkey1},"b",function()
        awful.spawn("rofi-bluetooth") end,
        { description="Bluetooth", group="Control" }
    ),
...
--        {description = "Wallpaper resume", group = "altkey"}),
   ...
   awful.key({ modkey,"Shift" }, "Tab", function () lain.util.tag_view_nonempty(-1) end,
    ...
    {description ="toggle wibox on current tag", group="awesome"}),
